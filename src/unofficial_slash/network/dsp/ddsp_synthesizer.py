...
def generate_periodic_excitation(
    ...
    # ハーモニクス次数
    harmonics = torch.arange(1, n_harmonics + 1, device=device).float()  # (n_harmonics,)
    
    # 各ハーモニクスの周波数: f0 * k
    harmonic_freqs = f0_safe.unsqueeze(-1) * harmonics.unsqueeze(0).unsqueeze(0)  # (B, T, n_harmonics)
    
    # 位相計算: 2π * freq * time / sample_rate
    phases = 2.0 * math.pi * harmonic_freqs.unsqueeze(-1) * time_indices / sample_rate  # (B, T, n_harmonics, frame_length)
    
    # ハーモニクス重み（高次ほど減衰）
    harmonic_weights = 1.0 / harmonics  # (n_harmonics,)
    harmonic_weights = harmonic_weights.unsqueeze(0).unsqueeze(0).unsqueeze(-1)  # (1, 1, n_harmonics, 1)
    
    # サイン波生成と重み付き合成
    sine_waves = torch.sin(phases) * harmonic_weights  # (B, T, n_harmonics, frame_length)
...
def generate_aperiodic_excitation(
    ...
    # ガウシアンノイズ生成
    noise = torch.randn(batch_size, time_frames, frame_length, device=device)  # (B, T, frame_length)
...
def apply_minimum_phase_response(
    ...
    # FFT変換
    fft_result = torch.fft.rfft(flat_excitation, n=frame_length, dim=-1)  # (B*T, freq_bins)
    ...
    symmetric_log_mag = torch.cat([
        log_magnitude, 
        torch.flip(log_magnitude[:, 1:-1], dims=[-1])
    ], dim=-1)  # (B*T, 2*freq_bins-2)
    
    # IFFT でケプストラム係数を計算
    cepstrum = torch.fft.irfft(symmetric_log_mag, n=frame_length, dim=-1)  # (B*T, frame_length)
...
class DDSPSynthesizer(nn.Module):
    ...
    def __init__(
        ...
        self.frame_length = frame_length or hop_length * 4  # FIXME: hop_length * 4 の妥当性検証が未完了
    ...
    def forward(
        ...
        spectrogram = torch.abs(stft_result).transpose(-1, -2)  # (B, freq_bins, T) -> (B, T, freq_bins)
        ...
        if spectrogram.shape[1] != time_frames:
            if spectrogram.shape[1] > time_frames:
                ...
                spectrogram = torch.cat([
                    spectrogram,
                    torch.zeros(batch_size, padding, spectrogram.shape[-1], device=device)
                ], dim=1)
    ...
    def generate_two_spectrograms(
        ...
        spectrogram_1 = self.forward(f0_values, spectral_envelope, aperiodicity, random_seed=42)
        spectrogram_2 = self.forward(f0_values, spectral_envelope, aperiodicity, random_seed=123)
