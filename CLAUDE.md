...
### **🔴 重要度：高（実学習実行への影響大）**
- ⚠️ **BAP損失の循環参照問題**: BAP予測値を使ってBAP損失ターゲットを生成している
  - `bap_mean = torch.mean(torch.sigmoid(bap), dim=-1)`でBAP自身を参照
  - 学習初期のBAP不安定性がV/UV判定精度に悪影響する可能性
  - より独立したV/UV判定基準（target_f0やスペクトル特徴）の検討が必要
- ⚠️ **BAP V/UVしきい値のハードコーディング**: `bap_mean < 0.5`の0.5が固定
  - 論文準拠の動的しきい値や学習可能パラメータへの変更検討
...
## 🎯 **次期実装優先順位** (Phase 5b以降 - 2025-08-07 更新)

### **🔴 高優先度** (実学習実行のために必須)
1. **BAP損失循環参照問題解決** - 独立したV/UV判定基準への変更
2. **BAP V/UVしきい値の設定ファイル化** - ハードコーディング0.5の動的化
...
### ⚠️ **実学習前に要解決項目**
- ⚠️ **BAP損失循環参照問題** - BAP予測値によるBAP損失ターゲット生成の改善
- ⚠️ **BAP V/UVしきい値設定化** - ハードコーディング0.5の動的化
...
**結論**: **Phase 5b完成により、主要技術課題が解決され実学習準備がほぼ完了**。残るBAP損失関連課題を解決すれば本格的な学習・評価が開始可能。
